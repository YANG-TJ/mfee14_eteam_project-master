...
function PartnersDetailPage(props) {
  ...
  const handleAddNewBlock = () => {
    console.log(menuTextField)
    const tempAddNewArr = [...menuTextField, { html: '', text: '' }]
    console.log(tempAddNewArr)
    setTextField(tempAddNewArr)
  }
  const handleDeleteBlock = (index) => {
    const tempDeleteLayouts = { lg: [], md: [], sm: [], xs: [], xxs: [] }
    for (let layout in menuLayoutsData) {
      menuLayoutsData[layout].forEach((e, i) => {
        if (i < index) tempDeleteLayouts[layout][i] = e
        if (i > index)
          tempDeleteLayouts[layout][i - 1] = { ...e, i: `${i - 1}` }
      })
    }
    const tempDeleteArr = [...menuTextField].filter((e, i) => i !== index)
  ...
  async function updateToSever() {
    ...
    const newData = { menuTextField, menuLayoutsData }
    console.log(menuTextField, menuLayoutsData)
    // 連接的伺服器資料網址
  ...
  const [preTextField, setPreTextField] = useState(menuTextField)
  const [preLayoutsData, setPreLayoutsData] = useState(menuLayoutsData)
  ...
  useEffect(() => {
    if (open) {
      // 抽屜開啟時儲存當前狀態
      setPreLayoutsData(menuLayoutsData)
      setPreTextField(menuTextField)
    }
    ...
    console.log('2.', menuLayoutsData)
    console.log('3.', preLayoutsData)
    console.log('4.', blockFlag)
    console.log('5.', menuTextField)
